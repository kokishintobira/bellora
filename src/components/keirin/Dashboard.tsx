...
import { aggregateStrategyResults, dailyResultForStrategy, formatRoi, formatYen, scalePerformance, STRATEGY_KEYS, STRATEGY_LABELS } from "@/lib/keirin/calculations";
...
export function Dashboard() {
  ...
  const selectedDailyResults = dailyResults.map((result) => dailyResultForStrategy(result, defaultStrategy));
  const monthKey = data.todayPredictions[0]?.date.slice(0, 7) ?? dailyResults[0]?.date.slice(0, 7) ?? "";
  const monthResults = dailyResults.filter((result) => result.date.startsWith(monthKey));
  const monthStrategies = STRATEGY_KEYS.map((key) => aggregateStrategyResults(monthResults, key));
  const month = scalePerformance(aggregateStrategyResults(monthResults, defaultStrategy), baseStake);
  const yesterday = selectedDailyResults[0] ? scalePerformance(selectedDailyResults[0], baseStake) : null;
  const strategyLabel = STRATEGY_LABELS[defaultStrategy];

  return (
    <>
      <PageHeading eyebrow="Performance overview" title="今月のシミュレーション成績" description={`${strategyLabel}・Workerが保存した確定済み実データ`} action={<StakePicker compact />} />
      <section className="k-kpi-grid" aria-label="今月の主要指標">
        ...
        <MetricCard label={`${strategyLabel} 回収率`} value={month.roi} type="roi" emphasized note="回収金 ÷ 投資額" />
      </section>
      <StrategyComparison strategies={monthStrategies} baseStake={baseStake} selectedStrategy={defaultStrategy} />
      ...
      <section className="k-dashboard-grid">
        <div className="k-card k-card-pad">
          ...
          <ProfitChart dailyResults={selectedDailyResults} />
        </div>
        <aside className="k-card k-yesterday">
          ...
          <div className="k-result-stats">
            ...
            <div><span>購入対象</span><b>{selectedDailyResults[0].betCount} / {selectedDailyResults[0].raceCount}R</b></div>
          </div>
      ...
      <section className="k-content-row">
        ...
        <div className="k-history-list">
          ...
          {selectedDailyResults.slice(0, 4).map((result) => {
