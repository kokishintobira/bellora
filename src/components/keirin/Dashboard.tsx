...
import { dailyResults, monthPerformance } from "@/lib/keirin/data";
...
export function Dashboard() {
  ...
  const yesterday = scalePerformance(dailyResults[0], baseStake);

  return (
    <>
      <PageHeading eyebrow="Performance overview" title="今月のシミュレーション成績" description="2026年8月1日 — 8月28日 ・ 結果確定済み" action={<StakePicker compact />} />
      <section className="k-kpi-grid" aria-label="今月の主要指標">
        <MetricCard label="投資額" value={month.investment} note="対象 125レース" />
        <MetricCard label="回収金" value={month.returnAmount} note="的中 34レース" />
        <MetricCard label="収支" value={month.profit} type="profit" note="前月比 +8,400円" />
        <MetricCard label="回収率" value={month.roi} type="roi" emphasized note="基準 100%　＋14.1pt" />
      </section>
      ...
      <section className="k-dashboard-grid">
        ...
        <aside className="k-card k-yesterday">
          <SectionHeading title="昨日の結果" subtitle="8月27日（木）" />
          <div className="k-result-hero"><span>回収率</span><strong>{formatRoi(yesterday.roi)}</strong><small>↗ 目標を20.0pt超過</small></div>
          ...
          <div className="k-result-footer"><Link className="k-link-arrow" href="/keirin/history">レース別結果を見る <ChevronIcon /></Link></div>
        </aside>
      </section>
        ...
        <div className="k-history-list">
          ...
          {dailyResults.slice(0, 4).map((result) => {
            ...
            return <Link key={result.id} href={`/keirin/history#${result.id}`} className="k-history-item">
              <div className="k-history-date"><span className="k-date-box"><span><small>AUG</small><b>{date.day}</b></span></span><span>{date.month}月{date.day}日（{date.weekday}）<small>{result.betCount}レース購入</small></span></div>
