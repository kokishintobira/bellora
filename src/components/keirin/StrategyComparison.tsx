...
import type { StrategyPerformance } from "@/lib/keirin/types";

export function StrategyComparison({ strategies, baseStake = 500, compact = false }: { strategies: StrategyPerformance[]; baseStake?: number; compact?: boolean }) {
  ...
  const selected = scaled.find((strategy) => strategy.key === "suitability_ab");
  ...
  return (
    <section className={`k-strategy-comparison ${compact ? "is-compact" : ""}`} aria-label="購入戦略の比較">
      ...
      <div className="k-strategy-table">
        ...
        {scaled.map((strategy) => <div className={`k-strategy-table-row ${strategy.key === "suitability_ab" ? "is-selected" : ""}`} key={strategy.key}>
